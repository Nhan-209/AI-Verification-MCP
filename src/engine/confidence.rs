...
impl ConfidenceAnalyzer {
    ...
    pub fn analyze(text: &str) -> ConfidenceReport {
        ...
        let expected_specifics = if word_count > 0.0 { word_count / 10.0 } else { 1.0 };
        ...
        for sentence in &sentences {
            ...
            for &neg in NEGATION_PREFIXES {
                if let Some(idx) = lower_sentence.find(neg) {
                    ...
                    let word_concept = concept.split_whitespace().take(3).collect::<Vec<_>>().join(" ");
        ...
        for concept in &negative_concepts {
            for pos in &positive_sentences {
                if pos.contains(concept) {
                    self_contradictions.push(format!("Contradiction found on concept: '{}'", concept));
                    break;
                }
            }
        }

        let contradiction_penalty = if !self_contradictions.is_empty() { 1.0 } else { 0.0 };
        ...
        let calibration_score = (groundedness - overconfidence_penalty - excessive_hedging_penalty).clamp(0.0, 1.0);
        ...
        if specificity < 0.4 {
            suggestions.push(
                "Include concrete technical references (code snippets, file paths, numbers, benchmarks).".to_string(),
            );
        }
...
mod tests {
    ...
    fn test_empirical_metric_not_overconfident() {
        let text =
            "We achieved 100% test coverage across all modules in tests/mcp_integration.rs. All 70 assertions passed.";
        let report = ConfidenceAnalyzer::analyze(text);
        assert_eq!(
            report.overconfidence_score, 0.0,
            "100% test coverage must not be flagged as overconfidence"
        );
        assert_eq!(report.verdict, "CALIBRATED");
    }
    ...
    fn test_contract_guaranteed_not_overconfident() {
        ...
        assert_eq!(
            report.overconfidence_score, 0.0,
            "Specification guarantees must not be flagged as overconfidence"
        );
        assert_eq!(report.verdict, "CALIBRATED");
    }
    ...
    fn test_cautious_negation_not_overconfident() {
        let text =
            "The tests do not prove the implementation is bug-free. The specification does not guarantee ordering.";
        let report = ConfidenceAnalyzer::analyze(text);
    ...
    fn test_evidence_laundering_prevented() {
        ...
        assert!(
            ...
            "Separate unverified sentence must not be laundered by top citation"
        );
        assert_eq!(report.verdict, "OVERCONFIDENT");
    }
